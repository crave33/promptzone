...
The script:
- ensures `PyInstaller` and `PySide6` exist in selected Python
- builds one-folder app at:
  - `dist\PromptZone\PromptZone.exe`
...
Do not commit personal data files:
- `Prompt_Library/`
- `settings.json`
- `tags.json`
- `weights.json`
...
Use `.gitignore` to keep repo clean and non-personal.
